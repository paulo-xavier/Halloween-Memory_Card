...
function getJumpscare() {
    ...
    }, 1000); // atraso antes de iniciar o jumpscare
}
...
function openRankingDialog() {
    ...
    gameData.forEach((player, index) => {
        ...
        if (player.id_game === currentPlayerId) {
            rankingPosition.style.backgroundColor = 'lightgray'; // destaca o jogador atual
        }
...
function editPlayerName() {
    ...
    if (newPlayerName) {
        updatePlayerName(newPlayerName); // atualiza o nome do jogador no backend
    }
}

// atualiza o nome do jogador no backend
async function updatePlayerName(newPlayerName) {
    const response = await fetch(`http://localhost:3000/game/${currentPlayerId}`, {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json',
    ...
    if (response.ok) {
        fetchRankingData(); // recarrega o dialogo de ranking para mostrar o novo nome
    } else {
        alert('Error updating player name.');
    }
}
...
async function deletePlayerData() {
    ...
    if (response.ok) {
        ...
        alert('Error deleting game data.');
    }
}
